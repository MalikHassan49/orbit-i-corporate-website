...
router.post(
  '/',
  authenticate,
  authorize(...CONTENT_EDITOR_ROLES),
...
router.patch('/:id', authenticate, authorize(...CONTENT_EDITOR_ROLES), validateBody(updateCategorySchema), asyncHandler(async (req, res) =>
  sendSuccess(res, 200, 'Category updated', await categoryService.update(req.params.id as string, req.body))))
router.delete('/:id', authenticate, authorize(...CONTENT_EDITOR_ROLES), asyncHandler(async (req, res) => {
...
tagRouter.post('/', authenticate, authorize(...CONTENT_EDITOR_ROLES), validateBody(createTagSchema), asyncHandler(async (req, res) =>
  sendSuccess(res, 201, 'Tag created', await tagService.create(req.body))))
tagRouter.patch('/:id', authenticate, authorize(...CONTENT_EDITOR_ROLES), validateBody(updateTagSchema), asyncHandler(async (req, res) =>
  sendSuccess(res, 200, 'Tag updated', await tagService.update(req.params.id as string, req.body))))
tagRouter.delete('/:id', authenticate, authorize(...CONTENT_EDITOR_ROLES), asyncHandler(async (req, res) => {
